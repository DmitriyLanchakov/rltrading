...
ts_argmax<-function(x, d){
    match(max(x,d),x)
}

#' which day ts_min(x, d) occurred on
ts_argmin<-function(x, d){
    match(min(x,d),x)
}
...
product<-function(x, d){
    rollapply(x, d, FUN = prod)
}
...
Alpha1<-function(returns, close,sdperiod=20,signedpow=2,tsmaxperiod=5){
    rank( ts_argmax(signedpower(ifelse(returns < 0 , 
                                       stddev(returns, sdperiod) , 
                                       close), 
                                signedpow), tsmaxperiod)) - 0.5
} 
