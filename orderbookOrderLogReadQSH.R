...
makeBidAsk<-function(orderlogrow, depth=3){
  orderbook<<-rbindlist(list(orderbook, orderlogrow))
  if(orderlogrow[,Fill]==TRUE){
...
tickbidaskdt<-orderlog[,makeBidAsk(.SD), by=id]
