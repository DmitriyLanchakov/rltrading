...
######### Orderflow ML strategy test ##############
#' Суть: есть SSA сигнал,который строится всему ценовому ряду, и в момент времени t
#' является предсказательным. Мы пытаемся его восстановить с помощью некскольких предикторов,
#' построенных по orderflow
...
period="1min"
symbol<-c("Ri")
symbdate<-as.Date("2015-09-18")
setwd("~/repos/Data/research/VTO")

assign(symbol, fread("RTS_1_18.09.2015.csv", stringsAsFactors = FALSE))
get(symbol)[,DT:=as.POSIXct(strptime(DT, format="%d.%m.%Y %H:%M:%OS"))]
setkey(get(symbol), DT)




# ##########################Data from MFD #############################
Ri<-Ri[DT>=as.POSIXct("2015-09-18")]

tickData<-getSymbol.MOEX(from=symbdate, symbdate,symbList = "RIZ5")
tickData[,DT:=dat_time]
setkey(tickData,DT)
tickData<-tickData[DT>=as.POSIXct("2015-09-18")]
data<-tickData[get(symbol), mult="last"]
data<-data[complete.cases(data)]
data<-data[,.SD[1],by=trade_id]

assign(symbol,xts(x=data[,.(price, amount, RESULT)], order.by = data$DT))
